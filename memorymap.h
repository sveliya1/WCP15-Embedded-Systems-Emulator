#ifndef MEMORYMAP_O_H
#define MEMORYMAP_O_H
...
public: 
	...
	uint16_t virtual read_halfword(uint32_t address)
	{
		if (endianness == ENDIAN_LITTLE)
			...
			return this->memory_array[address - offset] | (this->memory_array[address-offset + 1] << 8);
		else
			//return (read_byte(address) << 8) | read_byte(address + 1);
			return (this->memory_array[address - offset]) << 8 | (this->memory_array[address-offset + 1]);
	...
	uint32_t virtual read_word(uint32_t address)
	{
		if (endianness == ENDIAN_LITTLE)
		{
			...
			return this->memory_array[address - offset] |
				(this->memory_array[address - offset + 1] << 8) | (this->memory_array[address - offset + 2] << 16
					| (this->memory_array[address - offset + 3] << 24));
		}
		else
		{
			return (this->memory_array[address - offset] << 24) | (this->memory_array[address + 1 - offset]) << 16
				| (this->memory_array[address+2 - offset]) << 8 | (this->memory_array[address+3 - offset]);
	...
	void write_halfword(uint32_t address, uint16_t data)
	{
		if (endianness == ENDIAN_LITTLE)
		{
			this->memory_array[address-offset]= data & 0x00FF;
			this->memory_array[address + 1 - offset] = data >> 8;
		}
		else
		{
			this->memory_array[address - offset] = (data >> 8) ;
			this->memory_array[address+1 - offset] = data & 0x00FF;
	...
	void write_word(uint32_t address, uint32_t data)
	{
		...
		{
			this->memory_array[address - offset] = data & 0x00FF;
			this->memory_array[address + 1 - offset] = (data >> 8) & 0x00FF;
			this->memory_array[address + 2 - offset] = (data >> 16) & 0x00FF;
			this->memory_array[address + 3 - offset] = (data >> 24) & 0x00FF;
		...
		{
			this->memory_array[address - offset] = (data >> 24) & 0x00FF;
			this->memory_array[address + 1 - offset] = (data >> 16) & 0x00FF;
			this->memory_array[address + 2 - offset] = (data >> 8) & 0x00FF;
			this->memory_array[address + 3 - offset] = (data) & 0x00FF;
...
public:
	...
	void write_byte(uint32_t address, uint8_t data) { std::cout << "Write to flash failed, Address is " << address << std::endl; }
	void write_halfword(uint32_t address, uint16_t data) { ; }
	void write_word(uint32_t address, uint32_t data) { ; }
...
public:
	...
	*/
	void deviceNumber(uint64_t index)
	{
	...
	void write_byte(uint64_t index, uint32_t value)
	{
		for (auto& device : map)
		{
			//std::cout << "Offset is: " << device->getOffset() << std::endl;
			//std::cout << "End Address is : " << device->getEndAddress() << std::endl;
			if (index % (device->getEndAddress() + 1) == index) {
				device->write_byte(index, value);
				return;
			}
		}
	}
	void write_halfword(uint64_t index, uint32_t value)
	{
		for (auto& device : map)
		{
			//std::cout << "Offset is: " << device->getOffset() << std::endl;
	...
	void write_word(uint64_t index, uint32_t value)
	{
		for (auto& device : map)
		{
			//std::cout << "Offset is: " << device->getOffset() << std::endl;
