...
// could not parse:  196: 7999 ldrb r1, [r3, #6]
	cpsr flags = r1 and r2 ; // 198: 4211 tst r1, r2
...
// could not parse:  1a0: 400a ands r2, r1
	Z = ( ( r2 - 8 ) == 0 ) ; r2 > 8 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 1a2: 2a08 cmp r2, #8
...
// could not parse:  1aa: 7999 ldrb r1, [r3, #6]
	cpsr flags = r1 and r2 ; // 1ac: 4211 tst r1, r2
...
// could not parse:  1b8: 400a ands r2, r1
	Z = ( ( r2 - 12 ) == 0 ) ; r2 > 12 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 1ba: 2a0c cmp r2, #12
...
// could not parse:  1c6: 1813 adds r3, r2, r0
	Z = ( ( r3 - r1 ) == 0 ) ; r3 > r1 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 1c8: 428b cmp r3, r1
...
// could not parse:  1cc: 2100 movs r1, #0
	...
	Z = ( ( r3 - r2 ) == 0 ) ; r3 > r2 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 1d0: 4293 cmp r3, r2
...
// could not parse:  46c: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1134+0) /8 ] ; // 46e: 4a03 ldr r2, [pc, #12] ; (47c <toggle_green_led+0x10>)
...
// could not parse:  480: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1154+0) /8 ] ; // 482: 4a03 ldr r2, [pc, #12] ; (490 <turn_on_green_led+0x10>)
...
// could not parse:  494: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1174+0) /8 ] ; // 496: 4a03 ldr r2, [pc, #12] ; (4a4 <turn_off_green_led+0x10>)
...
// could not parse:  4a8: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1194+0) /8 ] ; // 4aa: 4a03 ldr r2, [pc, #12] ; (4b8 <toggle_red_led+0x10>)
...
// could not parse:  4bc: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1214+0) /8 ] ; // 4be: 4a03 ldr r2, [pc, #12] ; (4cc <turn_on_red_led+0x10>)
...
// could not parse:  4d0: 2380 movs r3, #128 ; 0x80
	r2 = Memory[ (r2+1234+0) /8 ] ; // 4d2: 4a03 ldr r2, [pc, #12] ; (4e0 <turn_off_red_led+0x10>)
...
Label: set_rgbled_color_to:
	...
	Z = ( ( r0 - 7 ) == 0 ) ; r0 > 7 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 516: 2807 cmp r0, #7
...
// could not parse:  570: 23a0 movs r3, #160 ; 0xa0
	r1 = Memory[ (r1+1394+0) /8 ] ; // 572: 4916 ldr r1, [pc, #88] ; (5cc <initialize_rgbled+0x5c>)
	r2 = Memory[ (r2+1396+0) /8 ] ; // 574: 4a16 ldr r2, [pc, #88] ; (5d0 <initialize_rgbled+0x60>)
	r3 = r3 << 5 ; // 576: 015b lsls r3, r3, #5
...
// could not parse:  5e8: 2001 movs r0, #1
	r3 = Memory[ (r3+1514+0) /8 ] ; // 5ea: 4b02 ldr r3, [pc, #8] ; (5f4 <sw1_is_pressed+0xc>)
	r3 = Memory[ (r3+r3+0) /8 ] ; // 5ec: 691b ldr r3, [r3, #16]
	r3 = r3 >> 20 ; // 5ee: 0d1b lsrs r3, r3, #20
// could not parse:  5f0: 4398 bics r0, r3
	R15 = lr ; T bit = Rn[0] ; // 5f2: 4770 bx lr
...
// could not parse:  60a: f7ff ffed bl 5e8 <sw1_is_pressed>
	Z = ( ( r0 - 0 ) == 0 ) ; r0 > 0 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 60e: 2800 cmp r0, #0
...
// could not parse:  612: f7ff fff1 bl 5f8 <sw1_is_not_pressed>
	Z = ( ( r0 - 0 ) == 0 ) ; r0 > 0 N = FaLsE ; } Else { N = TRUE } C = FaLsE //TODO V = FaLsE //TODO // 616: 2800 cmp r0, #0
...
// could not parse:  61c: 2380 movs r3, #128 ; 0x80
	r1 = Memory[ (r1+1566+0) /8 ] ; // 61e: 490b ldr r1, [pc, #44] ; (64c <configure_sw1+0x30>)
	r2 = Memory[ (r2+1568+0) /8 ] ; // 620: 4a0b ldr r2, [pc, #44] ; (650 <configure_sw1+0x34>)
...
Label: __gnu_thumb1_case_uqi:
	...
	r1 = lr ; // 666: 4671 mov r1, lr
	r1 = r1 >> 1 ; // 668: 0849 lsrs r1, r1, #1
	r1 = r1 << 1 ; // 66a: 0049 lsls r1, r1, #1
// could not parse:  66c: 5c09 ldrb r1, [r1, r0]
	r1 = r1 << 1 ; // 66e: 0049 lsls r1, r1, #1
	lr = lr + r1 ; // 670: 448e add lr, r1
// could not parse:  672: bc02 pop {r1}
	R15 = lr ; T bit = Rn[0] ; // 674: 4770 bx lr
